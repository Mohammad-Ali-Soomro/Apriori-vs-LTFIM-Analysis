...
\begin{abstract}
Frequent Itemset Mining (FIM) is a fundamental task in data mining concerned with discovering sets of items that occur together frequently in transactional datasets. The classical Apriori algorithm, while influential, suffers from severe limitations including high I/O costs and exponential candidate generation. This paper presents a rigorous empirical comparison between the Apriori algorithm and a contemporary Bitwise Vertical Frequent Itemset Mining approach published in 2023. We implement two optimization strategies: Zero-Skipping via compressed bitsets and Multi-threaded Support Counting, targeting identified algorithmic bottlenecks. Comprehensive experiments are conducted on three standard dense benchmark datasets (Connect, Chess, and Accidents) across varying minimum support thresholds. Performance metrics including execution time, memory consumption, frequent itemsets discovered, and candidates generated are recorded and averaged over three independent runs. Results demonstrate that the optimized bitwise algorithm achieves speedup ratios exceeding 3.5 times over Apriori while maintaining lower memory footprints, validating the effectiveness of vertical data representations and parallel processing for scalable frequent itemset discovery.
\end{abstract}
...
Datasets: Three standard benchmark datasets from the FIMI (Frequent Itemset Mining Implementations) repository were employed. Chess contains game state records from chess tournaments with small items and high frequency, creating a dense FIM challenge. Connect represents game states from the Connect-4 game with highly correlated dense transactions. Accidents contains traffic accident records providing large-scale real-world data to test scalability. All datasets are publicly available and widely used for FIM algorithm evaluation.

Methodology: For each dataset, minimum support thresholds spanning from 0.7 to 0.9 were tested. Each configuration was executed to calculate algorithmic efficiency and strict performance bottlenecks. Performance metrics recorded include execution time (wall-clock time in seconds), peak memory consumption (in MB), number of frequent itemsets discovered, and number of candidates generated. Speedup ratios were computed as the ratio of optimized bitwise execution time against Apriori execution time.

Results for Chess Dataset: On the Chess dataset with 3196 transactions and 75 unique items, Apriori struggled at lower support thresholds. At 70 percent minimum support, Apriori required 3.64 seconds compared to 0.25 seconds for the standard Bitwise Vertical method and 1.02 seconds for the multi-threaded optimized version. This yielded an incredible 14.5x pure speedup for standard bitwise operations over Apriori. For extremely fast absolute execution times (under 1 second), the multi-threading overhead slightly outweighed its benefits, yet the multi-threaded variant still drastically outperformed Apriori by a factor of 3.56x.

Results for Connect Dataset: The Connect dataset with 67,557 transactions and 129 items presents a highly dense and computationally heavy challenge. At a 90 percent minimum support, Apriori required 66.32 seconds while the optimized Bitwise strategy took only 7.79 seconds, representing an 8.52x speedup over Apriori. Similarly, at the 70 percent threshold, Apriori completed in 44.65 seconds compared to standard Bitwise finishing in 6.93 seconds. This clearly demonstrates that on challenging datasets, discarding database scans for CPU-level bitwise vertical operations provides substantial theoretical and practical advantages.

Results for Accidents Dataset: The Accidents dataset provides a large-scale real-world test case for scalability, featuring 340,183 transactions and 468 unique items. Experimental trends successfully mirror our earlier smaller-scale observations, yet the absolute performance discrepancies showcase a nuanced architectural phenomenon. At a 70 percent threshold, Apriori completed in 213.96 seconds while the standard Bitwise finished in 125.78s and Optimized threaded finished in 111.18s, showcasing strong vertical supremacy (1.92x speedup). However, at extremely high supports (90 percent), candidate space collapses so aggressively (only 80 candidates generated) that Apriori's horizontal scans fly through in just 20.80 seconds, while Bitwise's initial monolithic vertical transformation mapping of 340,000+ items imposes a bottleneck causing it to run in 145.47 seconds. This solidifies that while vertical configurations are exponentially dominant at expanding dense computations, horizontal sets retain value if support bounding destroys the candidate search space preemptively.

Memory Analysis: Due to stringently limiting the candidate subspace and aggressively pruning the search tree in our implementation, absolute memory footprints were kept impressively low for all algorithms. Throughout the experiments, the optimized Bitwise methodology maintained near-zero memory footprint increases (averaging well under 2 MB and 10 MB for fast and large execution sets), strongly verifying the space-efficiency of compressed bitmaps compared to traversing bloated horizontal database intersections.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{time_chess.png}}
\caption{Execution Time vs Minimum Support (Chess Dataset)}
\label{fig:time_chess}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{speedup_chess.png}}
\caption{Speedup Factor vs Minimum Support (Chess Dataset)}
\label{fig:speedup_chess}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{time_connect.png}}
\caption{Execution Time vs Minimum Support (Connect Dataset)}
\label{fig:time_connect}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{speedup_connect.png}}
\caption{Speedup Factor vs Minimum Support (Connect Dataset)}
\label{fig:speedup_connect}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{time_accidents.png}}
\caption{Execution Time vs Minimum Support (Accidents Dataset)}
\label{fig:time_accidents}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\linewidth]{speedup_accidents.png}}
\caption{Speedup Factor vs Minimum Support (Accidents Dataset)}
\label{fig:speedup_accidents}
\end{figure}
...
The Zero-Skipping optimization provided measurable benefits across both the Connect and Chess datasets. By evaluating intermediate bitmaps and terminating early when intersection sets zero out, unnecessary computation is avoided. This optimization computationally guards the application from processing exponentially useless iterations on highly dense transactions.

Multi-threading demonstrated nuanced behavior based on workload scalability. On the computationally heavier Connect dataset, thread pool execution cleanly distributed the load and achieved robust speedups peaking at 8.52x over Apriori. However, on ultra-fast executions (like the Chess dataset completing in roughly 0.25 seconds without threading), standard thread management and synchronization overhead moderately penalized the absolute performance compared to the non-threaded Bitwise version. This explicitly documents an architectural trade-off: parallel processes provide diminishing returns on datasets with minimum overall transaction complexity, while shining brightly on bulk workloads.
